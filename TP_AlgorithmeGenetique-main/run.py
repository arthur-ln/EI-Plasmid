...
def function_optimization(function, nEpochs, shape, pop_size, mutation_rate, crossover_rate, starting_interval, render=True):
    ...
    def fitness_function(x): return -function(x) * 1e-3

    algo_gen = AlgoGen(Individual_vector, pop_size, fitness_function,
                       shape, mutation_rate, crossover_rate, starting_interval)
    ...
    for e in tqdm(range(nEpochs)):
        ...
        avg_fitness.append(
            sum(indv.fitness_score for indv in algo_gen.population)/algo_gen.pop_size)
    ...
    print(
        f"Chromosomes du meilleur individu : {algo_gen.best_indv.chromosomes}\nValeur de la fonction pour cette individu : {result}")
...
def tsp(nEpochs, shape, pop_size, mutation_rate, crossover_rate, method='random', estimated_time=True, render=True):
    ...
    if method == 'random':
        cities = np.array([[random.random()*100, random.random()*100]
                          for _ in range(shape)])
    elif method == 'square':
        if (root_nCities := int(shape ** (1/2))) ** 2 == shape:
            cities = np.array([[x, y] for x in range(root_nCities)
                              for y in range(root_nCities)])
        else:
            raise ValueError(
                f"To use the square method the shape must be a perfect square : {shape}")
    else:
        NotImplementedError(
            f"The following method to create cities is not implemented : {method}")

    def fitness_function(x): return 1/(1+distance_path(cities, x))

    algo_gen = AlgoGen(Individual_path, pop_size,
                       fitness_function, shape, mutation_rate, crossover_rate)
...
if __name__ == "__main__":
    ...
    function_optimization(function=lvl1, nEpochs=100, shape=1, pop_size=100,
                          mutation_rate=0.2, crossover_rate=0.9, starting_interval=20)
